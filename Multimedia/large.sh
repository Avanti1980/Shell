...
echo "clip = mvf.BM3D(clip, sigma=[3,3,3], radius1=1)" >> $file_path.vpy
echo "clip = core.sub.TextFile(clip, file=r'"$file_name".ass')" >> $file_path.vpy
...
echo '开始压制 请耐心等待'
vspipe $file_path.vpy - --y4m | x264 --demuxer y4m --crf 22.0 --preset slow --tune film --keyint 600 --min-keyint 1 --input-depth 16 --sar 1:1 -o $file_path.vpy.264 -

echo '压制完成 合并视频和音频'
muxer -i $file_path.vpy.264?fps=30/1 -i $file_path.vpy.m4a -o $file_path.crf22.mp4
